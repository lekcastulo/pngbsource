<?php
		...
		class itemsInventory {
				...
				public function itemsFilterInventory($id, $dateFrom, $dateTo){
				require '../../connection.php';
					...
					$query = mysqli_query($connection, "
						...
						WHERE member.member_id ='$id' AND item_inventory.id = '$id' and date BETWEEN '$dateFrom' and '$dateTo'
						GROUP BY itemtype
						ORDER BY itemtype;
				...
				public function itemInventorySummary($id, $dateFrom, $dateTo, $itemid){
				require '../../connection.php';
				...
				if ($itemid == 'All') {
					...
					$query = mysqli_query($connection, "
						SELECT * FROM item_inventory
						WHERE id IN ('$id') AND DATE BETWEEN '$dateFrom' AND '$dateTo' and  item_id = '$itemid'
						ORDER BY DATE DESC
						");					
				}
