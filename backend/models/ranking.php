<?php
		...
		class totalRanking {
				...
				public function ranking($dateFrom, $dateTo){
					...
					SELECT member.firstname, sum(grams) as grams, DATE, member.member_id AS member, SUM(selling_price) AS Total_Sales
					FROM sales_report
					INNER JOIN member 
							...
							return $results;
                                ...
                                public function totalGrams($dateFrom, $dateTo){
                                        require '../../connection.php';
                                                ...
                                                $query = mysqli_query($connection, "select sum(grams) as totalgrams from sales_report where date between '$dateFrom' and '$dateTo'");
