<?php
	...
	if($_POST['type'] == 'editingCustomerRequest'){
		...
		if ($approval == 'Approved'){
			...
			UPDATE `script3`.`sales_report` SET `customer_name`='$customer' WHERE `id`='$member' and `official_receipt` = '$or'");
	...
	if($_POST['type'] == 'editingGramsRequest'){
		...
		if ($approval == 'Approved'){
			...
			$query = mysqli_query($connection, "
			UPDATE `script3`.`sales_report` SET `grams`='$grams' WHERE `id`='$member' and `official_receipt` = '$or'");
	...
	if($_POST['type'] == 'editingSellingpriceRequest'){
		...
		if ($approval == 'Approved'){
			...
			$query = mysqli_query($connection, "
			UPDATE `script3`.`sales_report` SET `selling_price`='$sellingprice' WHERE `id`='$member' and `official_receipt` = '$or'");
	...
	if($_POST['type'] == 'editingItemTypeRequest'){
		...
		if ($approval == 'Approved'){
			...
			$query = mysqli_query($connection, "
			UPDATE `script3`.`sales_report` SET `item_id`='$itemid',  `item_type`= '$changeitem', `item_value`= '$itemvalue' WHERE id = '$member' and official_receipt = '$or'	");
