...
<div class="col-md-12">		
 ...
 <?php 
	...
	<button name="search" class="btn btn-custom">Filter by date</button>
	<br><br><br>


	<div id="attendanceSummaryGrid"></div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('input[type=text][name=dateFrom], input[type=text][name=dateTo]').datepicker({
			format: 'yyyy-mm-dd',
			todayBtn: 'linked',
			autoclose: true,
		...
		function getAttendanceSummary() {
			var attendanceSummaryList = [];
			var dateFrom = $('input[type=text][name=dateFrom]').val();
			var dateTo = $('input[type=text][name=dateTo]').val();

			$.post('backend/controllers/memberTimeRecord.php', { requestType: 'getTimeRecordsWithComputationByDateRange', dateFrom: dateFrom, dateTo: dateTo }, function(data){
				attendanceSummaryList = JSON.parse(data);
				renderAttendanceSummaryData(attendanceSummaryList);
			});
		...
		function renderAttendanceSummaryData(attendanceSummaryList) {
			$('#attendanceSummaryGrid').jsGrid({
				width: '100%',
				editing: false,
				sorting: true,
				...
				fields: [
					...
					{ title: 'Total Minutes Late', name: 'totalMinutesLate', type: 'text' },
					{ title: 'Total Minutes Under Time', name: 'totalMinutesUnderTime', type: 'text' },
				],
		...
		function rowRenderer(item) {
			...
			var cellTotalLate = $('<td class="jsgrid-cell">'+parseInt(item.totalMinutesLate / 60)+' Hours and '+parseInt(item.totalMinutesLate % 60)+' Minutes</td>');
			var cellTotalUnderTime = $('<td class="jsgrid-cell">'+parseInt(item.totalMinutesUnderTime / 60)+' Hours and '+parseInt(item.totalMinutesUnderTime % 60)+' Minutes</td>');

			return row.append(cellMemberName).append(cellTotalLate).append(cellTotalUnderTime);
		}
